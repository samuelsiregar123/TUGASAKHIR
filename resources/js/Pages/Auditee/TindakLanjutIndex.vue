...
<template>
    <SidebarLayout title="Tindak Lanjut">
        ...
        <p class="page-sub">Daftar temuan audit dan status tindak lanjut</p>
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th style="width:40%">Instansi / Aplikasi</th>
                        <th style="width:18%; text-align:center">Total Temuan</th>
                        <th style="width:18%; text-align:center">Selesai</th>
                ...
                <tbody>
                    <tr v-for="(plan, i) in filtered" :key="plan.id">
                        ...
                        <td style="text-align:center">
                            <span v-if="plan.selesai > 0" class="chip-selesai">{{ plan.selesai }}/{{ plan.total_temuan }}</span>
                            <span v-else class="chip-none">{{ plan.total_temuan > 0 ? '0/'+plan.total_temuan : '—' }}</span>
                        </td>
                        <td style="text-align:center">
                            <button v-if="plan.total_temuan > 0" class="btn-link"
                                @click="router.visit(`/auditee/tindak-lanjut/${plan.id}`)">Buka</button>
                            <span v-else class="btn-link-off">—</span>
                        </td>
                    </tr>
                    <tr v-if="filtered.length === 0">
                        <td colspan="5" style="text-align:center; color:#aaa; padding:24px">Tidak ada data</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
.chip-none    { color:#aaa; font-size:.78rem; }
